...
\end{wrapfigure}
    ...
    Now consider the buffer zone introduced by segmental refinement, which can be represented as a new grid with two boundary conditions encoding the error. The boundary will thus be functions with fourier coefficients of k or greater since the coarser grid was accurate up to k coefficients. The  solution to this grid with boundary values of frequency k will decay to discretization error by the time it leaves the buffer zone. On this finer grid, then, we will have error of frequency 2k or higher, combined with the error that is introduced by the buffer zone. Overall, this function will still be accurate up to coefficients of 2k. 
